...
seo-description: 自动化Forms转化服务的常见问题
...
1. **自动化AEM Forms转换服务支持哪个版本的Forms?**

   <p>自动化Forms转换服务支持AEM 6.4Forms和AEM 6.5Forms。 它在OSGi和AEM JEE表单上都与AEM Forms协作。 您需要AEM作者实例顶部最新的AEM Forms加载项包才能使用服务。 For detailed instructions, see <a href="configure-service.md">Configure the Automated Forms Conversion</a> service.</p> 
    <br>

1. **该服务是否可以预先安装？**

   <p>Adobe定期训练自动Forms转换服务的AI和ML算法，使用新的数据集来提高转换精度。 更新的算法会定期部署到在Adobe云上运行的转换服务。 该服务的所有客户都从更新的算法中受益。 因此，云托管的中央部署最适合自动化的Forms转化服务，以持续学习和改进所有客户。</p> 
...
1. **该服务是否可以将Microsoft Word表单转换为自适应表单？**
   ...
   1. 使用Adobe Acrobat [将Word文档转换为非交互式PDF](https://helpx.adobe.com/acrobat/how-to/create-pdf-files-word-excel-website.html)。
   1. 使用Adobe Acrobat [将生成的PDF forms转换为可填写的PDF表单](https://helpx.adobe.com/acrobat/how-to/convert-word-excel-paper-pdf-forms.html)。
...
1. **该服务是否可以将扫描的纸质表单和彩色表单转换为自适应表单？**
   ...
   <p>该服务可以将颜色PDF forms转换为自适应表单。 服务不支持扫描或填写的表单。 有关其他限制，请参阅已 <a href="known-issues.md">知问题文</a> 章。</p> <br>
...
1. **该服务是否会从基于XDP的表单自动提取JavaScript并将其应用到相应的自适应表单？**
   ...
   <p>该服务不会自动将基于XFA的表单或AcroForms的脚本转换为相应的自适应表单规则。 您（表单作者）可以使用规则编 <a href="https://helpx.adobe.com/experience-manager/6-5/forms/using/rule-editor.html">辑器</a> ，将交互性添加到自适应表单。</p> <br>

1. **某些表单对象无法正确转换为自适应表单组件。 如何解决此问题？**

   <p>自动化的Forms转化服务通过一系列表单进行培训。 但基于AI/ML的应用程序受其培训数据和模式的限制。 可能有多种字段类型、布局、模式和上下文可以辨别为人类感知，但难以自动识别。 服务可能无法识别此类对象，或可能识别错误。 您可以使用“ <a href="review-correct-ui-edited.md" target="_blank">审阅”和“正确</a> ”编辑器，在熟悉的基于纸张表单的输入表单布局中进行必要的修改。</p> <br/>
