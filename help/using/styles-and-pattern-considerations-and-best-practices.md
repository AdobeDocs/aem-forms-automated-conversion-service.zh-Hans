---
title: 最佳实践和注意事项
description: 自动化表单转换服务(AFCS)的最佳实践和注意事项
...
本文档提供了管理员、作者和开发人员在使用[!DNL Automated Forms Conversion service] (AFCS)时可以从中受益的准则和建议。 该文档讨论了从准备源表单到修复复杂模式的最佳实践，这些模式需要为自动化转换付出一些额外的努力。 这些最佳做法共同对[!DNL Automated Forms Conversion service] (AFCS)的整体性能和输出做出了贡献。

## 最佳做法

转换服务将AEM [!DNL Forms]实例上可用的PDF forms转换为自适应表单。 下面列出的最佳实践可帮助您提高转换速度和准确性。 此外，这些最佳实践还可帮助您节省在转化活动后所花费的时间。
